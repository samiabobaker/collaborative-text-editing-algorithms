...
def transform_TIBOT_operation(O1: TIBOTOperation, O2: TIBOTOperation) -> tuple[TIBOTOperation, TIBOTOperation]:
    match O1.operation, O2.operation:
        case TIBOTInsertionOperation(i, x), TIBOTInsertionOperation(j, y):
            if i < j:
                transformed_O1 = TIBOTOperation(
                    O1.time_interval, O1.client_id, O1.sequence_number, TIBOTInsertionOperation(i, x)
                )
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTInsertionOperation(j + 1, y)
                )
            else:
                transformed_O1 = TIBOTOperation(
                    O1.time_interval, O1.client_id, O1.sequence_number, TIBOTInsertionOperation(i + 1, x)
                )
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTInsertionOperation(j, y)
                )
        case TIBOTInsertionOperation(i, x), TIBOTDeletionOperation(j, y):
            if i <= j:
                ...
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTDeletionOperation(j + 1, y)
                )
            else:
                transformed_O1 = TIBOTOperation(
                    O1.time_interval, O1.client_id, O1.sequence_number, TIBOTInsertionOperation(i - 1, x)
                ...
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTDeletionOperation(j, y)
                )
        case TIBOTDeletionOperation(i, x), TIBOTInsertionOperation(j, y):
            if i < j:
                transformed_O1 = TIBOTOperation(
                    O1.time_interval, O1.client_id, O1.sequence_number, TIBOTDeletionOperation(i, x)
                )
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTInsertionOperation(j - 1, y)
                )
                ...
                transformed_O1 = TIBOTOperation(
                    O1.time_interval, O1.client_id, O1.sequence_number, TIBOTDeletionOperation(i + 1, x)
                )
                transformed_O2 = TIBOTOperation(
                    O2.time_interval, O2.client_id, O2.sequence_number, TIBOTInsertionOperation(j, y)
                )
