...
class ABTClient(ClientDevice):
    ...
    def __is_causally_ready(self, message: ABTMessage) -> bool:
        ...
        for client in self.clients:
            if (
                ...
                and message.vector_clock[client.client_id] > self.vector_clock[client.client_id]
            ):
                return False
        return True
    ...
    def IT(self, O1: ABTOperation, O2: ABTOperation) -> ABTOperation | None:
        if O2.position < O1.position:
            ...
            if (
                isinstance(O2, ABTInsertOperation)
                and isinstance(O1, ABTDeleteOperation)
                or isinstance(O2, ABTInsertOperation)
                and isinstance(O1, ABTInsertOperation)
    ...
    def convert_to_HC(
        self, O: ABTOperation, vector_clock: dict[int, int], sequence: ABTSequence
    ) -> tuple[ABTSequence, ABTSequence]:
        ...
        for i in range(len(sequence)):
            if not self.happened_before(sequence[i], vector_clock):
                ...
                for k in range(len(sequence_c) - 1, -1, -1):
                    sequence[i].operation, sequence_c[k].operation = self.swap(
                        sequence[i].operation, sequence_c[k].operation
                    )
    ...
    def update_HL(
        ...
        for i in range(len(Hd) - 1, -1, -1):
            transformed_O.operation, transformed_Hd[i].operation = self.swap(
                transformed_O.operation, transformed_Hd[i].operation
            )
