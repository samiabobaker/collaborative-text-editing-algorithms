...
class AdOPTedTM11Client(ClientDevice):
    ...
    message_buffer: dict[int, list[AdOPTedTM11Message]]
    ...
    def __apply_operation(self, operation: AdOPTedTM11Operation) -> None:
        match operation:
            case AdOPTedTM11InsertionOperation(position, string):
                self.__insert_string(position, string)
            case AdOPTedTM11DeletionOperation(position, length):
                self.__delete_string(position, length)
            case AdOPTedTM11NoOperation():
                pass
    ...
    def __transform(self, O1: AdOPTedTM11Operation, O2: AdOPTedTM11Operation) -> AdOPTedTM11Operation:
        match O1, O2:
            case AdOPTedTM11InsertionOperation(i, x, pr1), AdOPTedTM11InsertionOperation(j, y, pr2):
            ...
            case AdOPTedTM11NoOperation(), _:
                return AdOPTedTM11NoOperation()
            case oper, AdOPTedTM11NoOperation():
                return oper
            case _ as unreachable:
                assert_never(unreachable)
