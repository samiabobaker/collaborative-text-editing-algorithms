...
class ModifiedExclusionTransformer(ExclusionTransformer):
    """
    A modified GOT Exclusion Transformer. The modifications against the original functions are described in Appendix C
    of the dissertation.

    Each function is not documented in detail. Refer to the detailed docstring comments in `ModifiedInclusionTransformer`.
    The concepts apply equally here.
    """
    ...
    def _et_insert_insert(self, op_a: GOTInsertOperation, op_b: GOTInsertOperation) -> GOTOperation:
        ...
        if op_a.idx <= op_b.idx:
            transformed_operation.save_precedes(op_b)
