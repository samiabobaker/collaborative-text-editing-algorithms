...
class WOOTClient(ClientDevice):
    ...
    def perform_local_insert(self, operation: ClientInsertOperation):
        ...
        character = WOOTCharacter(WOOTIdentifier(self.client_id, self.clock), operation.character, True, get_string_entry_id(prev), get_string_entry_id(next))
        self.integrate_insertion(character, get_string_entry_id(prev), get_string_entry_id(next))
        self.__send_to_other_clients(WOOTMessage(WOOTInsertionOperation(character.id, character.character, character.prev_id, character.next_id), operation))
    ...
    def lt_id(self, a: WOOTCharacter | Literal['start'] | Literal['end'], b: WOOTCharacter | Literal['start'] | Literal['end']) -> bool:
    ...
    def integrate_insertion(self, c: WOOTCharacter, prev_id: WOOTIdentifier, next_id: WOOTIdentifier):
        ...
        if len(substring) == 0:
            ...
            for character in substring:
                assert character != 'start' and character != 'end'
                if self.string.less_than_or_equal_to(character.prev_id, prev_id) and self.string.less_than_or_equal_to(next_id, character.next_id):
                    L.append(character)
