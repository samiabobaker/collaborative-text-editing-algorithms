...
class FugueMaxTree:
    ...
    def insert_char_at_node(self, parent_node_id: int, node_id: int, char: UniqueChar, direction: Literal['left','right'], right_origin_id: int | Literal['end'] | None) -> None:
    ...
    def __add_right_child(self, node: FugueMaxTreeNode) -> None:
        ...
        current_index = 0
        while current_index < len(self.right_children) and self.compare_right_origins(self.right_children[current_index].right_origin_id, node.right_origin_id, self.right_children[current_index].node_id, node.node_id):
    ...
    def compare_right_origins(self, right_id1: RightOriginId, right_id2: RightOriginId, node_id1: int, node_id2: int) -> bool:
...
class FugueMaxTreeNode:
    ...
    def __init__(self, tree: FugueMaxTree, node_id: int, value: UniqueChar, parent_node_id: int, direction: Literal['left','right'], right_origin_id: RightOriginId):
    ...
    def add_right_child(self, node: FugueMaxTreeNode) -> None:
        #Adds child to left_children while preserving order of ids.
        current_index = 0
        while current_index < len(self.right_children) and self.tree.compare_right_origins(self.right_children[current_index].right_origin_id, node.right_origin_id, self.right_children[current_index].node_id, node.node_id):
            #print(f"Comparing right origins of {self.right_children[current_index].value} and {node.value}")
            current_index += 1
    ...
    def copy(self, tree: FugueMaxTree) -> FugueMaxTreeNode:
        node_copy = FugueMaxTreeNode(tree, self.node_id, self.value, self.parent_node_id, self.direction, self.right_origin_id)
