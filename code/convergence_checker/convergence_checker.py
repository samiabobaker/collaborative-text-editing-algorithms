...
def check_for_convergence_client_server(server: ServerDevice, clients: dict[int, ClientDevice], num_of_operations:int = 10) -> bool:
    ...
    #Receive all messages into server
    ...
    for client_id in clients:
        client = clients[client_id]
        if client.read_state() != server_state:
...
def check_for_convergence_client_client_with_time_steps(clients: dict[int, TIBOTClient], num_of_operations:int = 10) -> bool:
    for _ in range(num_of_operations):
        operation = generate_random_client_client_operation(list(clients.values()))
...
def convergence_checker(clients: dict[int, ClientDevice], server: ServerDevice | None = None, num_of_ops:int=10) -> bool:
    if server:
