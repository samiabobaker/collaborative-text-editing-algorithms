...
from gottombstone.gottombstonemessage import GOTTombstoneMessage
...
from gottombstone.transformations.original.original_exclusion import OriginalExclusionTransformer
from gottombstone.transformations.original.original_inclusion import OriginalInclusionTransformer
from unique_char.uniquechar import UniqueChar


class GOTTombstoneClient(ClientDevice):
    client_id : int
    state: list[tuple[UniqueChar, int]]

    clients: list[GOTTombstoneClient]
    vector_clock: dict[int, int]
    message_buffer: dict[int, list[GOTTombstoneMessage]]

    _history_buffer : list[GOTTombstoneOperation]
    ...
    def get_state(self) -> list[UniqueChar]:
        return [c for c, v in self.state if v==0]
    ...
    def perform_local_insert(self, operation: ClientInsertOperation) -> GOTInsertOperation:
        ...
        """
        insert_operation = self._generate_insert(sequence=[operation.character], idx=self.view_to_model(operation.position))
        self._apply_operation_to_document(insert_operation)
        self.__send_to_other_clients(GOTTombstoneMessage(insert_operation, operation))
    ...
    def perform_local_delete(self, operation: ClientDeleteOperation) -> GOTDeleteOperation:
        ...
        """
        delete_operation = self._generate_delete(num_to_delete=1, idx=self.view_to_model(operation.position))
        self._apply_operation_to_document(delete_operation)
        self.__send_to_other_clients(GOTTombstoneMessage(delete_operation, operation))
    ...
    def _generate_delete(self, num_to_delete: int, idx: int) -> GOTDeleteOperation:
        ...
        return GOTDeleteOperation(
            num_to_delete,
            idx,
            sequence= [c for c, _ in self.state[idx : idx + num_to_delete]],
            client_id=self.client_id,
            state_vector=sv,
        )

    def send_message(self, client_id: int, message: GOTTombstoneMessage):
            self.message_buffer[client_id].append(message)
        
    def __send_to_other_clients(self, message: GOTTombstoneMessage) -> None:
    ...
    def set_clients(self, clients: list[GOTTombstoneClient]) -> None:
    ...
    def view_to_model(self, pview: int) -> int:
            n = 0
            j = 0
            while j < len(self.state) and (n<pview or self.state[j][1] != 0):
                if self.state[j][1] == 0:
                    n += 1
                j += 1
            return j
    ...
    def _apply_operation(self, incoming: GOTTombstoneOperation) -> bool:
            """
            ...
            if all_causally_ordered:
                ...
                if len(EOL) == 0:
                    ...
                    EOL_prime: list[GOTTombstoneOperation] = []
    ...
    def __is_causally_ready(self, incoming: GOTTombstoneOperation) -> bool:
    ...
    def _check_all_operations_causally_ordered(self, incoming_operation: GOTTombstoneOperation) -> tuple[bool, int]:
            """
    ...
    def _find_all_causally_ordered_operations_from(
            self, i: int, incoming_operation: GOTTombstoneOperation
        ) -> list[tuple[int, GOTTombstoneOperation]]:
            """
            ...
            incoming operation.
            """
            causally_ordered_ops: list[tuple[int, GOTTombstoneOperation]] = []
    ...
    def _undo_non_totally_ordered_operations(self, incoming_operation: GOTTombstoneOperation) -> list[GOTTombstoneOperation]:
            """
            ...
            """
            undone_operations : list[GOTTombstoneOperation] = []
    ...
    def _pop_history_buffer(self) -> GOTTombstoneOperation:
            """
    ...
    def _undo_operation(self, operation: GOTTombstoneOperation):
            """
            ...
            if isinstance(operation, GOTInsertOperation):
                num_inserted = len(operation.sequence)
                self.state = self.state[: operation.idx] + self.state[operation.idx + num_inserted:]
                
            elif isinstance(operation, GOTDeleteOperation):
                new_state = list(self.state)
                for i in range(operation.idx, operation.idx + operation.num_to_delete):
                    character, deletion_count = new_state[i]
                    new_state[i] = (character, deletion_count - 1)
                self.state = new_state
            else:
                raise ValueError("Unknown operation")
         
    def _apply_operation_to_document(
            self, operation: GOTTombstoneOperation, add_to_history_buffer: bool = True, increment_state_vector: bool = True
        ) -> None:
            """
    ...
    def _obtain_new_state(self, operation: GOTTombstoneOperation) -> list[tuple[UniqueChar, int]]:
            """
            ...
            if isinstance(operation, GOTInsertOperation):
                if len(operation.sequence) == 0:
                    return self.state
                new_state = self.state[: operation.idx] + [(c, 0) for c in operation.sequence] + self.state[operation.idx :]
            elif isinstance(operation, GOTDeleteOperation):
                new_state = list(self.state)
                for i in range(operation.idx, operation.idx + operation.num_to_delete):
                     character, deletion_count = new_state[i]
    ...
    def _redo_undone_operations(self, undone_operations: list[GOTTombstoneOperation], newly_performed: GOTTombstoneOperation) -> None:
            """
            ...
            """
            redone_ops: list[GOTTombstoneOperation] = []
