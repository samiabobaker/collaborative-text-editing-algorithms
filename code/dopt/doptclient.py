...
class dOPTClient(ClientDevice):
    ...
    def __perform_local_operation(self, operation: dOPTOperation, causing_operation: ClientInsertOperation | ClientDeleteOperation) -> None:
            self.request_log.append(dOPTRequestLogEntry(self.client_id, self.vector_clock.copy(), operation, operation.priority))
    ...
    def receive_from_client(self, client_id: int) -> list[ClientInsertOperation | ClientDeleteOperation]:
            ...
            while log_index != -1 and log_index < len(self.request_log):
                ...
                if message.vector_clock[log_entry.client_id] <= log_entry.vector_clock[log_entry.client_id]:
                    transformed_operation = dOPTTransform.transform_dOPT_operation(transformed_operation, log_entry.operation)
            ...
            self.request_log.append(dOPTRequestLogEntry(message.client_id, self.vector_clock.copy(), transformed_operation, transformed_operation.priority))
    ...
    def perform_local_insert(self, operation: ClientInsertOperation) -> None:
        self.__perform_local_operation(dOPTInsertionOperation(self.client_id, operation.position, operation.character), operation)
