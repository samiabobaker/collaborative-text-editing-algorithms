...
class YjsModClient(ClientDevice):
    """Peer to peer YjsMod client.
    ...
    guarantees an insertion's two anchors are already present when it is
    integrated, since the sender had them in its own document beforehand.
    """

    client_id: int

    document: YjsModDocument
    clients: list[YjsModClient]
    vector_clock: dict[int, int]
    message_buffer: dict[int, list[YjsModMessage]]

    __next_seq: int

    def __init__(self, client_id: int):
        self.document = YjsModDocument()
        self.client_id = client_id
        self.__next_seq = 0

    def set_clients(self, clients: list[YjsModClient]) -> None:
    ...
    def perform_local_insert(self, operation: ClientInsertOperation) -> None:
        ...
        id = YjsModId(self.client_id, self.__next_seq)
        self.__next_seq += 1
    ...
    def perform_remote_insert(
        ...
        self.document.integrate(YjsModItem(id, origin_left, origin_right, char))
    ...
    def __apply_operation(self, operation: YjsModOperation):
        match operation:
            case YjsModInsertionOperation(id, origin_left, origin_right, char):
                self.perform_remote_insert(id, origin_left, origin_right, char)
            case YjsModDeletionOperation(id):
    ...
    def send_message(self, client_id: int, message: YjsModMessage):
        self.message_buffer[client_id].append(message)

    def __send_to_other_clients(self, message: YjsModMessage) -> None:
        for client in self.clients:
    ...
    def __is_causally_ready(self, message: YjsModMessage) -> bool:
        for client in self.clients:
