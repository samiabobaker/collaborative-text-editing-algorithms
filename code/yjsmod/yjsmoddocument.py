...
class YjsModId:
    ...
    Like Yjs, YjsMod identifies items by (client, sequence number) rather than
    by a global counter, because the client component doubles as the tie
    breaker when two clients insert into exactly the same slot.
    """
...
class YjsModDocument:
    """The YjsMod sequence: every item ever inserted, held in document order.

    Deleted items stay in the list as tombstones so that concurrent operations
    which anchored to them can still be positioned.

    YjsMod is a modification of Yjs that resolves a conflict between two items
    sharing an origin_left by comparing their origin_right first, falling back
    to client id only when the two right anchors coincide. Yjs compares client
    id first. The practical effect is that a concurrent insertion no longer
    splits a run of characters typed by another client purely because that
    client's id happened to sort the wrong way.
    """

    items: list[YjsModItem]
    ...
    def __find_item(self, id: YjsModId | None) -> int:
    ...
    def insert_char(self, position: int, id: YjsModId, char: UniqueChar) -> YjsModItem:
    ...
    def delete_char(self, position: int) -> YjsModItem:
        index = self.__find_index_at_position(position)
    ...
    def delete_item_with_id(self, id: YjsModId) -> None:
        self.__mark_deleted(self.items[self.__find_item(id)])

    def integrate(self, new_item: YjsModItem) -> None:
        """Place new_item in the sequence - the YjsMod integration rule.
        ...
        while True:
            ...
            if other_left < left:
                ...
                # Same anchor. Unlike Yjs, the right anchors decide first.
                if other_right < right:
                    # Its window is narrower than ours, so it is nested inside
                    # our range. We might end up after it, but a later sibling
                    # could still overrule that - remember this spot and keep
                    # looking.
                    scanning = True
                elif other_right == right:
                    # Both anchors identical: a raw conflict, broken by client
                    # id.
                    ...
                    # Its window is wider than ours, so we belong inside it.
                    scanning = False
            else:
    ...
    def __mark_deleted(self, item: YjsModItem) -> None:
        if not item.deleted:
            item.deleted = True
            self.count -= 1
