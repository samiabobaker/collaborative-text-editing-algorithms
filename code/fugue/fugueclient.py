...
class FugueClient(ClientDevice):
    ...
    def perform_local_insert(self, operation: ClientInsertOperation) -> None:
        ...
        self.__send_to_other_clients(FugueMessage(self.vector_clock.copy(), FugueInsertionOperation(node.parent_node_id, node.node_id, node.value, node.direction), operation))

    def perform_local_delete(self, operation: ClientDeleteOperation) -> None:
        #Perform change to local tree
        node = self.tree.delete_char(operation.position)
        #Send message to all other clients
        self.__send_to_other_clients(FugueMessage(self.vector_clock.copy(), FugueDeletionOperation(node.node_id), operation))

    def perform_remote_insert(self, parent_node_id: int, node_id: int, char: UniqueChar, direction: Literal['left','right']) -> None:
