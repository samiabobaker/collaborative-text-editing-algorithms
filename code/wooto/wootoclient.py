...
class WOOTOClient(ClientDevice):
    ...
    def perform_local_insert(self, operation: ClientInsertOperation):
        ...
        character = WOOTOCharacter(
            WOOTOIdentifier(self.client_id, self.clock),
            operation.character,
            True,
        ...
        self.__send_to_other_clients(
            WOOTOMessage(
                WOOTOInsertionOperation(
                    character.id, character.character, character.prev_id, character.next_id, character.degree
                ),
                operation,
            )
        )
    ...
    def lt_id(
        self,
        a: WOOTOCharacter | Literal["start"] | Literal["end"],
        b: WOOTOCharacter | Literal["start"] | Literal["end"],
    ) -> bool:
    ...
    def receive_from_client(self, client_id: int) -> list[ClientInsertOperation | ClientDeleteOperation]:
        ...
        match message.operation:
            ...
            case WOOTOInsertionOperation(id, character, prev_id, next_id, degree):
                self.integrate_insertion(
                    WOOTOCharacter(id, character, True, prev_id, next_id, degree), prev_id, next_id
                )
            case _ as unreachable:
                assert_never(unreachable)
