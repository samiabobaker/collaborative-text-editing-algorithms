...
class TIBOT2Client(TimeSteppedClient):
    ...
    def perform_local_insert(self, operation: ClientInsertOperation):
        op = TIBOT2Operation(self.clock, self.client_id, self.sequence_number, TIBOT2InsertionOperation(operation.position, operation.character))
    ...
    def perform_local_delete(self, operation: ClientDeleteOperation):
        op = TIBOT2Operation(self.clock, self.client_id, self.sequence_number, TIBOT2DeletionOperation(operation.position, operation.character))
        self.sequence_number += 1
        self.perform_local_operation(op)
        self.causing_operations[self.clock].append(operation)
    ...
    def perform_remote_group_operation(self, group_operation: list[TIBOT2Operation]):
        ...
        for operation in self.history_buffer:
            if operation.time_interval < group_operation[0].time_interval:
                L1.append(operation)
            elif operation.time_interval == group_operation[0].time_interval and operation.client_id < group_operation[0].client_id:
                L2.append(operation)
            else:
                L3.append(operation)
    ...
    def at_end_of_time_interval(self) -> None:
        ...
        if len(self.operation_buffer[time_interval]) == 0 or self.can_send_message_from_time_interval(time_interval):
            message = TIBOT2Message(self.client_id, time_interval, self.operation_buffer[time_interval], self.causing_operations[time_interval])
