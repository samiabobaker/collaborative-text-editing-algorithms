...
class SOCT3Client(ClientDevice):
    ...
    def perform_operation(self, operation: ClientOperation) -> list[ClientInsertOperation | ClientDeleteOperation]:
        match operation:
            case ClientInsertOperation(client_id, position, character):
                self.execute_local(
                    SOCT3InsertOperation(
                        self.vector_clock.copy(), -1, self.client_id, character, self.view_to_model(position), operation
                    ),
                    operation,
                )
                return [operation]
            case ClientDeleteOperation(client_id, position):
                self.execute_local(
                    SOCT3DeleteOperation(
                        self.vector_clock.copy(), -1, self.client_id, self.view_to_model(position), operation
                    ),
                    operation,
                )
                return [operation]
    ...
    def __advance(self):
        while (
            len(self.history_buffer) > self.timestamp
            and self.history_buffer[self.timestamp].timestamp == self.timestamp + 1
        ):
            self.timestamp += 1
        ...
        for j in range(self.timestamp, len(self.history_buffer)):
            if self.history_buffer[j].timestamp == -1:
                ...
                self.__send_to_other_clients(
                    SOCT3OperationMessage(self.client_id, message.operation, message.causing_operation)
                )
                break
        self.__advance()
