...
class JupiterServer(ServerDevice):
    ...
    def __send_operation_to_other_clients(
        self,
        client_view_from: ClientView,
        ...
        for client_id in self.clients:
            ...
            message = JupiterMessage(
                client_view.client_message_count, client_view.server_message_count, operation, causing_operation
            )
    ...
    def __apply_message(self, client_view: ClientView, received_message: JupiterMessage) -> None:
        client_view.outgoing = [
            m for m in client_view.outgoing if m.server_message_count >= received_message.server_message_count
        ]

        transformed_operation = received_message.operation

        for index, message in enumerate(client_view.outgoing):
            transformed_operation, transformed_outgoing_operation = JupiterTransform.transform_jupiter_operation(
                transformed_operation, message.operation
            )
            client_view.outgoing[index] = JupiterMessage(
                message.client_message_count,
                message.server_message_count,
                transformed_outgoing_operation,
